...
class CommonHttpClient:
    ...
    def post(self, body):
        res = requests.post(self.endpoint, data=json.dumps(body), headers=self.headers)
        return res.json()


if __name__ == "__main__":
    print("--------------------GET Repuest--------------------")
    http_get_request = CommonHttpClient(endpoint="https://httpbin.org/get")
    print(http_get_request.get({"param1": "hoge", "param2": 1}))
    http_post_request = CommonHttpClient(endpoint="https://httpbin.org/post")
    print("--------------------POST Repuest--------------------")
